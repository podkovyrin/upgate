...
impl Sandbox {
    ...
    fn run_fake_yarn(&self, args: &[&str]) -> Output {
        ...
        cmd.output().expect("failed to run fake yarn")
    }
...
fn deterministic_plan_covers_ready_delayed_pinned_and_error_states() {
    ...
    assert!(out.contains("- Skipped [yarn] pinned-pkg v3.0.0 -> v3.0.0 (pinned)"));
...
fn deterministic_scan_uses_fake_installed_state_and_reports_release_age_metadata() {
    ...
    assert!(
        out.contains("= Current [yarn] scan-noage v5.0.0 (source: yarn)"),
        "scan stdout:\n{out}\nscan stderr:\n{err}"
    );
}
...
fn hybrid_apply_uses_real_registry_time_data_with_fake_installed_state() {
    ...
    let Some(real_yarn_path) = sandbox.find_real_yarn() else {
        panic!("hybrid test requires real yarn in PATH");
    };

    let real_yarn_path = real_yarn_path.to_string_lossy().into_owned();
    ...
    assert!(
        out.contains("- Skipped [yarn] eslint v1.0.0 -> v1.0.0 (pinned)"),
        "hybrid stdout:\n{out}\nhybrid stderr:\n{err}"
    );
    assert!(
        out.contains("~ Delayed [yarn] react v9999.0.0 -> v"),
        "hybrid stdout:\n{out}\nhybrid stderr:\n{err}"
    );
    assert!(
