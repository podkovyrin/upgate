...
pub const fn select_less_stable_class(
...
pub fn matches_any_label(label: &str, labels: &[&str]) -> bool {
    labels.contains(&label)
}

pub fn leading_alpha_prefix(token: &str) -> &str {
    let end = token
        .char_indices()
...
pub const RC_LABELS: &[&str] = &["prerelease", "pre", "rc"];
