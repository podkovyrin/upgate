...
Managers may use min-release-age when it is an input to a native resolver command, such as `uv --exclude-newer` or `mise --before`. Managers must not receive `now` or compare timestamps against policy age for planning, update discovery, or verbose scan evidence discovery. Clock-aware policy decisions belong to planning, and user-visible scan release-age calculation belongs to app/CLI orchestration after managers return typed release evidence.
...
## Testing Strategy
The test suite is intentionally small and hostile to incidental coverage. Keep tests only at stable product boundaries: CLI behavior, public API contracts, durable domain invariants, manager product behavior, version-policy behavior, real-world parsing, user config behavior, important error handling, and integration paths that are expensive or risky to validate manually.

Do not add tests for private helpers, constructor/getter/setter plumbing, implementation order, mock-call counts, current module boundaries, internal view-model shape, temporary architecture scaffolding, or formatter/render helpers unless the exact output is a user contract. Do not add snapshots or goldens by default; use them only when they encode a real user-visible contract and would be intentionally preserved across a redesign.

Fixtures are allowed only when their malformed or real-world input shape is itself the behavior under test. Delete fixtures once their consuming stable-boundary tests are removed. Prefer one strong integration or boundary test over many unit tests that mirror production logic. Every retained or new test needs the proof: `This test must exist because it protects <specific stable behavior> that would otherwise be at meaningful regression risk.`
