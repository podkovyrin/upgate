...
mod tests {
    ...
    #[test]
    fn progress_screen_confirms_stop_after_current_without_string_state() {
        let stop_requested = AtomicBool::new(false);
        let mut screen = InteractiveProgressScreen::new(ExecutionProgressState {
            rows: vec![row(
                "pnpm",
                "pnpm:alpha",
                "alpha",
        ...
        screen.apply_event(ExecutionProgressEvent::manager_started(
            ManagerId::new("pnpm").expect("valid manager id"),
        ));
        screen.handle_input(ProgressInput::Quit, &stop_requested);
        assert!(screen.quit_confirmation_open());
        assert!(!stop_requested.load(Ordering::Relaxed));

        screen.handle_input(ProgressInput::ConfirmQuitAfterCurrent, &stop_requested);

        assert!(!screen.quit_confirmation_open());
        assert!(stop_requested.load(Ordering::Relaxed));
        assert!(screen.state().stop_after_current);
    }

    #[test]
    fn progress_screen_can_cancel_quit_confirmation() {
        let stop_requested = AtomicBool::new(false);
        let mut screen = InteractiveProgressScreen::new(ExecutionProgressState {
            rows: vec![row(
                "npm",
    ...
    #[test]
    fn live_progress_success_rows_do_not_render_command_notes() {
        let mut screen = InteractiveProgressScreen::new(ExecutionProgressState {
            rows: vec![row(
                "npm",
                "npm:beta",
        ...
        screen.apply_event(ExecutionProgressEvent::Finished);

        let rendered = render_to_string(&mut screen, 100, 12);

        assert!(rendered.contains("Apply complete: ok"), "{rendered}");
        assert!(rendered.contains("[✔]"), "{rendered}");
        assert!(!rendered.contains("npm update -g beta"), "{rendered}");
    }

    #[test]
    fn live_progress_renders_failure_skipped_and_manager_failure_rows() {
        let mut screen = InteractiveProgressScreen::new(ExecutionProgressState {
            rows: vec![
                row(
                    "brew",
                ...
                row(
                    ...
                    "delta",
                    ExecutionProgressStatus::Skipped {
                        detail: "stop requested".to_owned(),
                    },
                ),
            ],
            manager_failures: Vec::new(),
        ...
        screen.apply_event(ExecutionProgressEvent::manager_failed(
            ManagerId::new("npm").expect("valid manager id"),
            "command construction failed".to_owned(),
        ));
        screen.apply_event(ExecutionProgressEvent::Finished);

        let rendered = render_to_string(&mut screen, 120, 14);

        assert!(rendered.contains("Apply complete: failed"), "{rendered}");
        assert!(rendered.contains("[⨯]"), "{rendered}");
        assert!(
            rendered.contains("brew upgrade gamma: failed"),
            "{rendered}"
        );
        assert!(rendered.contains("[-]"), "{rendered}");
        assert!(rendered.contains("stop requested"), "{rendered}");
        assert!(
            rendered.contains("command construction failed"),
            "{rendered}"
        );
    }

    #[test]
    fn live_progress_renders_stopped_summary() {
        let mut screen = InteractiveProgressScreen::new(ExecutionProgressState {
            rows: vec![row(
                "pnpm",
                "pnpm:alpha",
        ...
        screen.apply_event(ExecutionProgressEvent::StopAfterCurrentRequested);
        screen.apply_event(ExecutionProgressEvent::Finished);

        let rendered = render_to_string(&mut screen, 100, 12);

        assert!(rendered.contains("Apply complete: stopped"), "{rendered}");
        assert!(rendered.contains("[-]"), "{rendered}");
        assert!(rendered.contains("stopped after current"), "{rendered}");
    }
    ...
    fn render_to_string(screen: &mut InteractiveProgressScreen, width: u16, height: u16) -> String {
        let backend = TestBackend::new(width, height);
        let mut terminal = Terminal::new(backend).expect("test terminal");
        terminal
            .draw(|frame| draw_progress(frame, screen))
