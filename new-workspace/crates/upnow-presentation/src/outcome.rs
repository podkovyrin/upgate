...
mod tests {
    ...
    #[test]
    fn version_label_adds_v_to_numeric_versions() {
        assert_eq!(version_label("1.2.3"), "v1.2.3");
        assert_eq!(version_label("v1.2.3"), "v1.2.3");
        assert_eq!(version_label("latest"), "latest");
    }

    #[test]
    fn changed_version_segment_detection_ignores_v_prefix() {
        assert_eq!(changed_version_segment_index("v1.2.3", "1.2.3"), None);
        assert_eq!(changed_version_segment_index("1.2.3", "1.3.0"), Some(1));
        assert_eq!(changed_version_segment_index("1.2", "1.2.1"), Some(2));
        assert_eq!(changed_version_segment_index("1.2.3", "2.0.0"), Some(0));
    }

    #[test]
    fn ansi_codes_are_removed_before_width_calculation() {
        let table = OutcomeTable::new(vec![
            item_row("npm", "short", "1.0.0", "1.2.0"),
            item_row("npm", "much-longer-name", "2.0.0", "2.1.0"),
        ]);

        let rendered = render_outcome_table(&table, OutputTheme::styled(true, false));
        let plain = strip_ansi_codes(&rendered);
        let lines = plain.lines().collect::<Vec<_>>();

        assert_ne!(rendered, plain);
        assert_eq!(lines[1].find("v1.0.0"), lines[2].find("v2.0.0"));
        assert_eq!(lines[1].find("v1.2.0"), lines[2].find("v2.1.0"));
    }

    #[test]
    fn plain_target_rendering_is_identity() {
        let rendered = render_to_version("v1.2.3", "v1.3.0", OutputTheme::plain(false), true);

        assert_eq!(rendered, "v1.3.0");
    }

    #[test]
    fn color_target_rendering_highlights_changed_segments() {
        let rendered =
            render_to_version("v1.2.3", "v1.3.0", OutputTheme::styled(true, false), false);

        assert!(rendered.contains("\u{1b}[34m"));
        assert!(!rendered.contains("\u{1b}[1m"));
        assert_eq!(strip_ansi_codes(&rendered), "v1.3.0");
    }

    #[test]
    fn status_labels_match_old_outcome_prefixes() {
        let table = OutcomeTable::new(vec![
            item_row_with_status(OutcomeStatusView::Current),
            item_row_with_status(OutcomeStatusView::Update),
            item_row_with_status(OutcomeStatusView::Delayed),
            item_row_with_status(OutcomeStatusView::Skipped),
            item_row_with_status(OutcomeStatusView::Error),
        ]);

        let rendered = render_outcome_table(&table, OutputTheme::plain(false));

        assert!(rendered.contains("= Current"));
        assert!(rendered.contains("+ Update"));
        assert!(rendered.contains("~ Delayed"));
        assert!(rendered.contains("- Skipped"));
        assert!(rendered.contains("! Error"));
    }

    #[test]
    fn table_aligns_widths_and_omits_version_arrow() {
        let table = OutcomeTable::new(vec![
            item_row("npm", "short", "1.0.0", "1.2.0"),
            item_row("npm", "much-longer-name", "2.0.0", "2.1.0"),
        ]);
    ...
    #[test]
    fn table_omits_empty_optional_columns_for_manager_rows() {
        let row = OutcomeRow::manager(OutcomeStatusView::Skipped, manager_id("cargo")).with_note(
            OutcomeNote::normal("(required command 'cargo' is not available)"),
        );
        let table = OutcomeTable::new(vec![row]);
    ...
    #[test]
    fn verbose_only_rows_are_hidden_until_verbose() {
        let table = OutcomeTable::new(vec![
            item_row("npm", "foo", "1.0.0", "1.2.0")
                .with_visibility(OutcomeVisibility::VerboseOnly),
        ]);

        assert_eq!(render_outcome_table(&table, OutputTheme::plain(false)), "");
        assert!(render_outcome_table(&table, OutputTheme::plain(true)).contains("foo"));
    }

    #[test]
    fn verbose_only_notes_are_hidden_until_verbose() {
        let table = OutcomeTable::new(vec![
            item_row("npm", "foo", "1.0.0", "1.2.0").with_note(
                OutcomeNote::metadata("(latest too fresh: 3d < 7d)")
                    .with_visibility(OutcomeVisibility::VerboseOnly),
            ),
        ]);

        let normal = render_outcome_table(&table, OutputTheme::plain(false));
        let verbose = render_outcome_table(&table, OutputTheme::plain(true));

        assert!(!normal.contains("latest too fresh"));
        assert!(verbose.contains("latest too fresh: 3d < 7d"));
    }

    #[test]
    fn colored_table_uses_plain_widths_for_alignment() {
        let table = OutcomeTable::new(vec![
            item_row("npm", "short", "1.0.0", "1.2.0"),
            item_row("npm", "much-longer-name", "2.0.0", "2.1.0"),
        ]);
    ...
    fn item_row(manager: &str, name: &str, from: &str, to: &str) -> OutcomeRow {
        OutcomeRow::item(
            OutcomeStatusView::Update,
            manager_id(manager),
            package_name(name),
            OutcomeVersionsView::change(version(from), version(to)),
        )
    }

    fn item_row_with_status(status: OutcomeStatusView) -> OutcomeRow {
        OutcomeRow::item(
            status,
            manager_id("npm"),
            package_name("alpha"),
