...
mod tests {
    use super::*;
    use upnow_domain::{
        CandidateEvaluationFact, ExecutionEligibility, ManagerMetadata, PackageName, PlanItemId,
        PolicyBlockReason, ToolId, ToolName, VersionScheme, VersionText,
    };

    #[test]
    fn scan_verbose_renders_adaptive_table_with_release_age() {
        let manager_id = manager_id("npm");
        let report = ScanReport::new(
            manager_id,
            vec![ScanItem::InstalledWithReleaseAge {
                tool: tool("npm", "alpha", "1.0.0"),
                age: Duration::from_secs(8 * 24 * 60 * 60),
            }],
            Vec::new(),
        );

        let rendered = render_scan_report(
            &report,
            BatchRenderOptions::new(OutputTheme::plain(true))
                .with_old_age_threshold(Duration::from_secs(7 * 24 * 60 * 60)),
        );

        assert_eq!(
            rendered,
            "Status     Manager  Name   Current  Note\n= Current  [npm]    alpha  v1.0.0   (released: 8d)\n"
        );
    }

    #[test]
    fn plan_current_rows_are_verbose_only() {
        let manager_id = manager_id("npm");
        let plan = UpdatePlan::new(
            manager_id,
            vec![PlanItem::Current {
                id: PlanItemId::new("npm:alpha").expect("valid id"),
                installed: tool("npm", "alpha", "1.0.0"),
            }],
        )
        .expect("valid plan");

        assert_eq!(
            render_update_plan(&plan, BatchRenderOptions::new(OutputTheme::plain(false))),
            ""
        );

        let rendered = render_update_plan(&plan, BatchRenderOptions::new(OutputTheme::plain(true)));
        assert!(rendered.contains("= Current"));
        assert!(rendered.contains("(no newer version found)"));
    }

    #[test]
    fn plan_update_renders_old_style_status_and_versions() {
        let manager_id = manager_id("pnpm");
        let candidate = UpdateCandidate::new(
            ToolId::new("pnpm:alpha").expect("valid tool id"),
            PackageName::new("alpha").expect("valid package"),
        ...
        let plan = UpdatePlan::new(
            manager_id,
            vec![PlanItem::Update {
                id: PlanItemId::new("pnpm:alpha").expect("valid id"),
                candidate,
            }],
        )
        .expect("valid plan");
        ...
        assert_eq!(
            rendered,
            "Status    Manager  Name   Current  Target\n+ Update  [pnpm]   alpha  v1.0.0   v1.2.0\n"
        );
    }

    #[test]
    fn policy_config_alone_does_not_render_blocked_latest_note() {
        let manager_id = manager_id("pnpm");
        let candidate = UpdateCandidate::new(
            ToolId::new("pnpm:alpha").expect("valid tool id"),
            PackageName::new("alpha").expect("valid package"),
        ...
        let rendered = render_update_plan(
            &plan,
            BatchRenderOptions::new(OutputTheme::plain(false))
                .with_version_policy(VersionPolicy::Stable),
        );

        assert!(!rendered.contains("blocked by version policy"));
    }

    #[test]
    fn policy_blocked_candidate_renders_blocked_latest_note() {
        let manager_id = manager_id("pnpm");
        let candidate = UpdateCandidate::new(
            ToolId::new("pnpm:alpha").expect("valid tool id"),
            PackageName::new("alpha").expect("valid package"),
        ...
        .with_diagnostics(PlanDiagnostics {
            required_age: Duration::ZERO,
            candidates: vec![CandidateEvaluationFact {
                version: VersionText::new("2.0.0-beta.1").expect("valid version"),
                age: Some(Duration::from_secs(30 * 24 * 60 * 60)),
        ...
        assert!(rendered.contains("(latest v2.0.0-beta.1 blocked by version policy: stable)"));
    }

    fn tool(manager: &str, package: &str, version: &str) -> InstalledTool {
        InstalledTool::new(
            manager_id(manager),
            ToolId::new(format!("{manager}:{package}")).expect("valid tool id"),
