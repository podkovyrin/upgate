...
fn adapter_uses_native_selected_update_for_no_policy_unforced_selection() {
    ...
    let selection = selection_for_plan(&plan, false);
    let execution_plan = resolve(&plan, &selection, VersionPolicy::None);
...
fn adapter_uses_exact_install_for_policy_selection() {
    ...
    let selection = selection_for_plan(&plan, false);
    let execution_plan = resolve(&plan, &selection, VersionPolicy::Stable);
...
fn adapter_forced_delayed_selection_uses_exact_install_and_bypasses_min_age() {
    ...
    let selection = selection_for_plan(&plan, true);
    let execution_plan = resolve(&plan, &selection, VersionPolicy::None);
