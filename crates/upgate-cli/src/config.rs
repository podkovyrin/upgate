...
use upgate_managers::adapter::ManagerConfigRuleError;
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidManagerConcurrency { value } => {
                write!(
                    formatter,
                    "invalid [upgate].manager_concurrency `{value}`, expected a value greater than 0"
                )
            }
            Self::InvalidAuditConcurrency { value } => {
                write!(
                    formatter,
                    "invalid [upgate].audit_concurrency `{value}`, expected a value from 1 to 16"
                )
            }
...
impl ConfigFile {
    ...
    pub fn set_manager_concurrency(&mut self, value: usize) -> Result<(), ConfigError> {
        validate_manager_concurrency(value)?;
        self.upgate.manager_concurrency = Some(value);
        Ok(())
    }
        ...
        if section == "upgate" {
            return match key {
                ...
                "audit_concurrency" => {
                    ...
                    self.upgate.audit_concurrency = Some(parsed);
                    Ok(())
                }
