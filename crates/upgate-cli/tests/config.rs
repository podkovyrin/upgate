...
fn missing_config_uses_global_defaults() {
    ...
    let config = ConfigFile::load_from_path(&path).expect("missing config should load defaults");
...
fn config_rejects_invalid_modes_and_npm_subday_age() {
    let config: ConfigFile = toml::from_str("[npm]\nmode = \"scan\"\n").expect("TOML should parse");
    assert!(matches!(
        config.resolve_manager("npm"),
...
fn cli_overrides_reject_unknown_and_non_phase_five_values() {
    ...
    assert!(matches!(
        config.apply_cli_override("upgate.manager_concurrency=0"),
